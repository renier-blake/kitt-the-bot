...
async function main() {
  ...
  try {
    ...
    await element.fill(input.text, { timeout: config.timeouts.action });
    success('Filled field', { ref: input.ref, text: input.text });
  } catch (e) {
    error(e instanceof Error ? e.message : 'Failed to fill field');
    process.exit(1);
  }
}
